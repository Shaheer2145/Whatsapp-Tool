...
export async function sendBroadcastMessage(req: Request, res: Response) {
  const { groupName, contacts, message } = req.body;
  // const options = req.body.options || {};

  if (!groupName || !Array.isArray(contacts) || !message) {
    return res.status(400).json({ error: 'Missing required fields' });
  }
  try {
    const DBdata = await Broadcastmessage.create({
      groupName,
      contacts,
      message,
    });
    ...
    return res.status(200).json({ Sucess: true, data: DBdata });
  } catch (error) {
    console.error('Error saving broadcast messages:', error);
    return res.status(404).json({ error: 'Internal server error' });
  }
}

export async function getBroadcastMessage(req: Request, res: Response) {
  try {
    const DBdata = await Broadcastmessage.find({});
...
export async function getTodayBroadcastMessage(re: Request, res: Response) {
  try {
    const date = new Date();
    const start = new Date().setHours(0, 0, 0, 0);
    const end = new Date().setHours(23, 59, 59, 999);
    const todayMessage = await Broadcastmessage.find({
      createdAt: {
        $st: start,
        $et: end,
      },
    });
    return res.status(200).json({ Sucess: true, data: todayMessage });
  } catch (error) {
    return res.status(400).json({ error: ' Internal Server Error ' });
  }
}

export async function updateBroadCastMessage(req: Request, res: Response) {
  const id = req.params.id;
  const { groupName, contacts, message } = req.body;
  if (!id || !groupName || !contacts || !message) {
    res.status(400).json({ error: 'Missing required fields' });
  }
  try {
    const updateDB = await Broadcastmessage.findByIdAndUpdate(
      id,
      {
        groupName,
        contacts,
    ...
    if (!updateDB) {
      res.status(404).json({ error: 'Broadcast not found updated' });
    }
    res.json({
      message: 'BroadCast Message has been edited successfully',
      data: updateDB,
    });
  } catch (err) {
