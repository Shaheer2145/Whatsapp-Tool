    ...
    return (
        <Layout>
            <SearchComponent style={{marginBottom: 0}}>
                ...
                <input placeholder={"Search Contact"} onChange={(e) => onSearch(e)}/>{" "}
                <Search/>
            </SearchComponent>
                                            ...
                                            <p className={"contact-name"}>
                                                {contact.name === undefined
                                                    ? (typeof contact?.id === 'string'
                                                        ? contact.id.replace("@c.us", "").replace("@g.us", "")
                                                        : '')
                                                    : contact.name}
                                            </p>
                                            <div className={"contact-message"}>
                                                {(()=>{
                                                    const lastMsg = contact.msgs?.[contact.msgs.length-1];
                                                    if(!lastMsg) return "Could not load previous messages...";
                                                    const mediaTypes= ['image','video','file','ptt','sticker'];
                                                    if(mediaTypes.includes(lastMsg.type)){
                                                        return "Media Message";
                                                    }
                                                    if(lastMsg.type==='revoked'){
                                                        return "Deleted Message";
                                                    }
                                                    if(lastMsg.type === 'gp2' || lastMsg.type === 'notification_template'){
                                                        return "No messages";
                                                    }
                                                    return lastMsg.body;
                                                })()}
