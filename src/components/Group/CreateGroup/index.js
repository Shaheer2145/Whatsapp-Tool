...
function ModalCreateGroup({ open, handleClose ,getAllGroups ,addGroup}) {
    ...
    async function onSubmitForm() {
        ...
        if (groupName !== "" && tagsChoosed.length > 0 && description !== "") {
            try {
                ...
                console.log("Group has been created", data);

                const groupInfo = data?.response.groupInfo;
                console.log("GroupInfo", groupInfo);

                if (!Array.isArray(groupInfo)) {
                    throw new Error("Groupinfo is not an array");
                }

                for (const groupInfoElement of groupInfo) {
                    if (!groupInfoElement?.id) {
                        console.log('id is missing ', groupInfoElement);
                        continue;
                    }
                    try {
                        ...
                        console.log("Group description has been settled", groupInfoElement.id);

                        const newGroup = groupInfo.map(group=>({
                            id:group.id,
                            name:groupName,
                            participants:tagsChoosed
                        }));
                        ...
                        console.log('phone:', groupInfoElement.id);
